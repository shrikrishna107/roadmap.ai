...
export default function AuthPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#0B0B0B" }}>
        <div className="text-center">
          <div className="w-16 h-16 border-4 rounded-full animate-spin mx-auto" style={{ borderColor: "#A259FF", borderTopColor: "transparent" }}></div>
          <p className="mt-4" style={{ color: "#A259FF" }}>Redirecting to dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error instanceof Error) setError(error.message);
      else setError('An error occurred with Google sign-in');
    } finally {
      setLoading(false);
    }
  };
